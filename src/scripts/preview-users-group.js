// TODO: criar a documentacao da funcao previewUserGroup
/**
 * @param {string} xPathDivModal
 * @param {string} classDivWrapperAvatarNome
 * @param {string} xPathSpanNomeGrupo
 * @param {string} xPathDivCriacaoGrupo
 * @returns 
 */
export async function previewUserGroup(
  ...
  function getDateCreatedGroup() {
    try {

      let span_date_group = document.evaluate(xPathDivCriacaoGrupo, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
      if (span_date_group.singleNodeValue) {      
        let content = span_date_group.singleNodeValue.textContent;
        return content.replace(/\<.*\>/gi, "");
      } else {
      ...
      //const span_date_group = document.querySelector(DIV_CRIACAO_GRUPO).firstChild;
  ...
  function card(contact) {
    ...
    return `<div class="content"> <img src="${url}" /> <div class="info" data-number="${contact.number}"> <p>${contact.number}${admin}</p> <p>${contact.name}</p> <p>${contact.status}</p> </div> </div>`;
  }
  ...
  let html = `<title>${getNameGroup()}</title>${STYLE}<h2>${getNameGroup()} (${
    contacts.length
  } participantes, <small>${getDateCreatedGroup()}</small>)</h2>`;
  let cards = "";
  let i = 1;
